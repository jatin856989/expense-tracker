...
const SYSTEM_PROMPT = `You extract investment holdings from a screenshot of a portfolio/investment app (Zerodha Coin, Groww, Kuvera, a brokerage or mutual fund statement, or similar). Respond with ONLY a JSON object, no other text.
...
Also return "platform": the name of the app/broker shown in the screenshot (e.g. "Zerodha Coin", "Groww"), or null if unclear.

Numbers may be abbreviated or use commas — convert to plain numbers. "₹12.53k" means 12530. "7,550.78" means 7550.78.

If the image doesn't look like a portfolio/investment screen at all, return exactly: {"platform": null, "holdings": []}

Respond with exactly this shape:
{"platform": <string or null>, "holdings": [{"name": <string>, "symbol": <string or null>, "instrumentType": <one of the types above>, "investedAmount": <number or null>, "currentValue": <number or null>, "units": <number or null>, "purchasePrice": <number or null>}]}`;

export type AnalyzeImageResult =
  | { success: true; data: PortfolioImageResult }
